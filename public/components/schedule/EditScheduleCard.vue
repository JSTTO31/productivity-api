<template>
    <v-form @submit.prevent="submit">
        <div class="d-flex mb-1 flex-column">
            <v-text-field prepend-inner-icon="mdi-calendar-text" density="compact" :error-messages="$v.title.$errors[0]?.$message.toString() || ''" class="mb-1" variant="solo-filled" flat  v-model="$v.title.$model"  placeholder="What's the agenda?" />
        </div>
        <!-- <v-text-field v-model="schedule.title" flat label="Title of schedule" single-line variant="solo-filled" density="compact" prepend-icon="mdi-calendar-text"></v-text-field> -->
...
const schedule = reactive({
    ...
    endAt: props.schedule?.endAt || new Date(),
